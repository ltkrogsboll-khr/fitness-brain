...
DEFAULTS = {
    ...
    "coach": {
        ...
        # Which LLM answers, and how to reach it -- see llm.py. Nothing here
        # defaults to a vendor: `provider` and `model` are blank until you set
        # them, so an unconfigured install has chat disabled rather than
        # quietly talking to whoever the author happened to use.
        #
        # `provider` picks a request/response *shape*, not literally who's
        # hosting it: "anthropic" is the Messages API, "openai" is the Chat
        # Completions API that OpenAI itself, Ollama, LM Studio and most
        # gateways all speak. Blank `base_url` / `api_key_env` fall back to
        # the shape's own default (api.anthropic.com / ANTHROPIC_API_KEY, or
